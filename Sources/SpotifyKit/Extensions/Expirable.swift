...
public struct Expirable<Value> {
    ...
    /// Uses given duration only for this value set (global duration isn't overwritten)
    public mutating func set(_ newValue: Value, duration: TimeInterval) {
        let expirationDate = Date().addingTimeInterval(duration)
        set(newValue, expirationDate: expirationDate)
    }
}
